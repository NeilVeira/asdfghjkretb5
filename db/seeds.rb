...
people_list = [
	["FirstName1", "LastName1", "dob1", "phone1","email1"],["FirstName2", "LastName2", "dob2",  "phone2","email2"],["FirstName3", "LastName3", "dob3", "address3", "phone3","email3"],
["FN3", "LN3", "dob3",  "phone3","email3"],["FN4", "LN4", "dob4",  "phone4","email4"],["FN5", "LN5", "d5",  "ph5","em5"],["FN6", "LN6", "d6",  "ph6","em6"]
]

people_list.each do |firstname,lastname,dob,phone,email|
	Person.create(firstname: firstname, lastname: lastname, dob: dob, phone: phone, email: email)
end

golfcourse_list = [
	"golfn1","golfn2","golfn3","golfn4","golfn5","golfn6","golfn7"
]

golfcourse_list.each do |name|
...
registered_people_list = [
	[Person.first.id,Tournament.first.id],[Person.second.id,Tournament.first.id],[Person.third.id,Tournament.third.id],[Person.fourth.id,Tournament.first.id]
]

registered_people_list.each do | person_id, tournament_id|
	RegisteredPerson.create(person_id: person_id, tournament_id: tournament_id)
end
